...
freguesias_pois <- freguesias |>
  select(dtmnfr) |>
  st_drop_geometry()
municipios_pois <- municipios |>
  select(municipio) |>
  st_drop_geometry()
grid_pois <- grid |>
  select(id) |>
...
impt_write(freguesias_pois, "pois/freguesias_pois.csv")
impt_write(municipios_pois, "pois/municipios_pois.csv")
impt_write(grid_pois, "pois/grid_pois.csv")
