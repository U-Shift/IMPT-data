...
Safety_bike <- Safety |> select(dtmnfr, contains("bici")) |>
...
acc_car_scores  <- data.frame(dtmnfr = Accessibility_car_Norm$dtmnfr,
  Accessibility_Index = scale_0_100(as.numeric(pca_car_access$ind$coord[, 1])))
acc_pt_scores   <- data.frame(dtmnfr = Accessibility_pt_Norm$dtmnfr,
  Accessibility_Index = scale_0_100(as.numeric(pca_pt_access$ind$coord[, 1])))
acc_bike_scores <- data.frame(dtmnfr = Accessibility_bike_Norm$dtmnfr,
  Accessibility_Index = scale_0_100(as.numeric(pca_bike_access$ind$coord[, 1])))
acc_walk_scores <- data.frame(dtmnfr = Accessibility_walk_Norm$dtmnfr,
...
aff_car_scores         <- Affordability_car_Norm |> select(dtmnfr, h_transp_inc_car) |>
  rename(Affordability_Index = h_transp_inc_car)
aff_pt_scores_navegante <- Affordability_pt_Norm_navegante |> select(dtmnfr, h_transp_inc_pt) |>
  rename(Affordability_Index_navegante = h_transp_inc_pt)
aff_pt_scores_singlefare <- Affordability_pt_Norm_singlefare |> select(dtmnfr, h_transp_inc_pt) |>
  rename(Affordability_Index_singlefare = h_transp_inc_pt)

# Per-mode mobility scores
mob_car_scores  <- data.frame(dtmnfr = Mobility_car_Norm$dtmnfr,
  Mobility_Index = scale_0_100(pca_car_mobility$ind$coord[, 1]))
mob_pt_scores   <- data.frame(dtmnfr = Mobility_pt_Norm$dtmnfr,
  Mobility_Index = scale_0_100(pca_pt_mobility$ind$coord[, 1]))
mob_bike_scores <- data.frame(dtmnfr = Mobility_bike_Norm$dtmnfr,
  Mobility_Index = scale_0_100(pca_bike_mobility$ind$coord[, 1]))
mob_walk_scores <- data.frame(dtmnfr = Mobility_walk_Norm$dtmnfr,
