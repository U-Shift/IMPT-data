...
grid_commuting_sf <- grid |>
  select(id, geom) |>
  left_join(grid_commuting, by = c("id" = "id_grid_origin"))
...
freguesia_commuting_sf <- freguesias |>
  select(dtmnfr, geom) |>
  left_join(freguesia_commuting, by = c("dtmnfr" = "Origin_dicofre24"))
...
municipio_commuting_sf <- municipios |>
  select(municipio, geom) |>
  left_join(municipio_commuting, by = c("municipio" = "municipio"))
...
ttm_transfers <- ttm_4t |>
  rename(tt_4t = travel_time_p50) |>
...
grid_transfers_sf <- grid |>
  select(id, geom) |>
  left_join(grid_transfers, by = c("id" = "id_grid_origin"))
...
freguesia_transfers_sf <- freguesias |>
  select(dtmnfr, geom) |>
  left_join(freguesia_transfers, by = c("dtmnfr" = "Origin_dicofre24"))
...
municipio_transfers_sf <- municipios |>
  select(municipio, geom) |>
  left_join(municipio_transfers, by = c("municipio" = "municipio"))
