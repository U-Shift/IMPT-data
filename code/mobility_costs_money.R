...
compute_tools_for_itinerary = function(sf_itinerary) {
  ...
  geometry_array = sf_itinerary$geometry[[1]] |> st_coordinates() |> as.data.frame() |> select(X, Y) |> as.matrix() |> toJSON(auto_unBOX = FALSE)
...
freguesia_commuting_money_sf = freguesias |> select(dtmnfr, geom) |> mutate(dtmnfr = as.integer(dtmnfr)) |> left_join(freguesia_commuting_money, by=c("dtmnfr" = "Origin_dicofre24"))
