...
The Accessibility dimension quantifies the capacity to reach key destinations within a given travel time threshold. This dimension incorporates two metrics:

1.  Cumulative number of opportunities: number of points of interest of a given category reachable within a certain amount of time. @tbl-accessibility_n_opportunities shows the travel time threshold taken into account given the different points of interest and modes.
2.  Travel time to reach closest opportunities: time required to reach the first *n* opportunities of a given category (1st opportunity for healthcare, basic education and green spaces; 3rd opportunity for recreation, groceries and jobs).
...
knitr::kable(accessibility_n_opportunities, caption = "Travel time threshold (minutes) to compute cumulative number of opportunities", align = c("l", "c", "c", "c", "c", "c"))
```
