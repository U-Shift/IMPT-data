...
Stop-level service frequencies were computed using `tidytransit::get_stop_frequency()` [@tidytransit] for four different time windows, as can be seen in <!--# tabela -->
...
The full results of the IMPT for all LMA parishes are available in the dashboard created in the scope of this project. This dashboard allows for the visualization of the global index, the results of each dimension by transport mode, all indicators that make up the index, as well as key statistics and summaries for various scales (parish, municipality and grid). Beyond results visualization, this tool allows for the calculation of the aggregated index adjusted to user preferences (political IMPT). For this purpose, an Analytic Hierarchical Process (AHP), that compares the user's preferences between dimensions and this way assigns weights to each, is made available to the dashboard's users. This dashboard is available at: <https://ushift.tecnico.ulisboa.pt/apps/impt>.
