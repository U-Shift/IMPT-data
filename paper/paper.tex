...
\subsection{Definitions of Transport
Poverty}\label{definitions-of-transport-poverty}
...
\subsection{Dimensions of Transport
Poverty}\label{dimensions-of-transport-poverty}
...
\subsection{IMPT's Contribution}\label{impts-contribution}
...
and creation of both the index and its interactive dashboard: readr,
readxl, openxlsx, lubridate, dplyr, tidyr, stringr, tidyverse, sf,
terra, mapview, stplanr, h3jsr, r5r, accessibility, odjitter,
tidytransit, GTFShift, FactoMineR, assertthat, piggyback.

\subsection{Each Dimension's Indicators}\label{sec-indicators}
...
\subsubsection{Accessibility}\label{accessibility-1}
...
\paragraph{Jittering}\label{jittering}
...
\citep{INE_IMOB2017} with centroid to centroid representations, ``a
common approach involving the simplifying assumption that all trip
destinations and origins can be represented by (sometimes population
weighted or aggregated) zone centroids'' \citep{Lovelace_2022}. However,
in order to accurately compute accessibility and mobility measures,
there was the need to have more disaggregated data regarding job
locations, beyond the parish unit.
...
\paragraph{Travel time matrix}\label{travel-time-matrix}
...
The matrix was computed using ()

\paragraph{Cumulative number of
opportunities}\label{cumulative-number-of-opportunities}

The cumulative number of opportunities was calculated () considering the
travel time matrices for each grid cell combination, the number of
opportunities per grid cell, and the travel time cut-offs.
...
\NormalTok{)}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(accessibility\_n\_opportunities, }\AttributeTok{align =} \FunctionTok{c}\NormalTok{(}\StringTok{"l"}\NormalTok{, }\StringTok{"c"}\NormalTok{, }\StringTok{"c"}\NormalTok{, }\StringTok{"c"}\NormalTok{, }\StringTok{"c"}\NormalTok{, }\StringTok{"c"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
opportunities}\label{travel-time-to-reach-closest-opportunities}

This measure was computed () considering the travel time matrices for
each grid cell combination and the number of opportunities in each grid
cell. Its output corresponds to the travel cost, in minutes, to reach
\emph{n} opportunities of a certain category. Although calculations were
performed for 1, 2 and 3 opportunities, only some of these were included
in IMPT's operationalization, depending on the POI category:
...
service, independently of the accessibility of the destinations. Its
principal indicators are (\ldots)

\subsubsection{Affordability}\label{affordability-1}
...
\paragraph{Cost of driving}\label{cost-of-driving}

\paragraph{Cost of public
transportation}\label{cost-of-public-transportation}

\paragraph{Composite measures}\label{composite-measures}

\subsubsection{Safety}\label{safety-1}
...
are systematized in Table~\ref{tbl-safety_indicators}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{safety\_indicators }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \StringTok{"Indicator"} \OtherTok{=} \FunctionTok{c}\NormalTok{(}\StringTok{"Total accidents (5 years)"}\NormalTok{,}
                  \StringTok{"Fatalities (30{-}day)"}\NormalTok{,}
                  \StringTok{"Serious injuries (30{-}day)"}\NormalTok{,}
                  \StringTok{"Slight injuries (30{-}day)"}\NormalTok{,}
                  \StringTok{"Accident rate"}\NormalTok{,}
                  \StringTok{"Fatality rate"}\NormalTok{,}
                  \StringTok{"Severity index (total)"}\NormalTok{,}
                  \StringTok{"Severity index (motorized)"}\NormalTok{,}
                  \StringTok{"Severity index (cycling)"}\NormalTok{,}
                  \StringTok{"Severity index (walking)"}
\NormalTok{                  ),}
  \StringTok{"Formula"} \OtherTok{=} \FunctionTok{c}\NormalTok{(}\StringTok{"Count"}\NormalTok{,}
                \StringTok{"sum(VM\_30d)"}\NormalTok{,}
                \StringTok{"sum(FG\_30d)"}\NormalTok{,}
                \StringTok{"sum(FL\_30d)"}\NormalTok{,}
                \StringTok{"total\_accidents/population × 1000"}\NormalTok{,}
                \StringTok{"fatalities/population × 1000"}\NormalTok{,}
                \StringTok{"fatalities/total\_victims"}\NormalTok{,}
                \StringTok{"fatalities/motorized\_vehicles\_involved"}\NormalTok{,}
                \StringTok{"fatalities/cycling\_involved"}\NormalTok{,}
                \StringTok{"fatalities/pedestrians\_involved"}\NormalTok{),}
  \AttributeTok{check.names =} \ConstantTok{FALSE}
\NormalTok{)}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(safety\_indicators)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{tbl-safety_indicators}Travel time threshold (minutes) to
compute cumulative number of opportunities}
...
Fatalities (30-day) & sum(VM\_30d) \\
Serious injuries (30-day) & sum(FG\_30d) \\
Slight injuries (30-day) & sum(FL\_30d) \\
Accident rate & total\_accidents/population × 1000 \\
Fatality rate & fatalities/population × 1000 \\
Severity index (total) & fatalities/total\_victims \\
Severity index (motorized) & fatalities/motorized\_vehicles\_involved \\
Severity index (cycling) & fatalities/cycling\_involved \\
Severity index (walking) & fatalities/pedestrians\_involved \\

\end{longtable}

\subsection{Normalisation and Aggregation}\label{sec-normalisation}
